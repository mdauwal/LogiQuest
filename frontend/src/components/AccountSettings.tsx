...
const AccountSettings: React.FC<AccountSettingsProps> = ({ onSave }) => {
  ...
  return (
    <div className={`w-full min-h-screen transition-colors duration-200 ${
      settings.theme.lightMode ? 'bg-white text-black' : 'bg-[#121212] text-gray-300'
    }`}>
      {/* Header */}
      <header className={`p-4 border-b flex flex-col md:flex-row items-center justify-between transition-colors duration-200 ${
        ...
        <nav className="flex space-x-4 md:space-x-6 items-center w-full md:w-auto overflow-x-auto 
          ...
          <button 
            className={`px-2 pb-2 whitespace-nowrap ${
              activeTab === 'profile' ? 'text-white text-[15px] md:text-[17px] border-b-2 border-[#F9BC07]' : 'text-gray-400'
            }`}
          ...
          <button 
            className={`px-2 pb-2 ${activeTab === 'account' ? 'text-[#F9BC07] border-b-2 text-[17px] border-[#F9BC07]' : 'text-gray-400'}`}
          ...
          <button 
            className={`px-2 pb-2 ${activeTab === 'game' ? 'text-white border-b-2 border-[#F9BC07] text-[17px]' : 'text-gray-400'}`}
      ...
      {/* Main Content */}
      <main className={`p-4 md:p-6 max-w-3xl mx-auto transition-colors duration-200 ${
        settings.theme.lightMode ? 'bg-white' : 'bg-[#121212]'
      }`}>
        {/* Notification Section */}
        <section className="mb-6 px-4 md:px-0" aria-labelledby="notification-title">
          <h2 id="notification-title" className={`text-[24px] md:text-[28px] mb-1 ${
            settings.theme.lightMode ? 'text-black' : 'text-[#CFFDED]'
          }`}>
            Notification
          </h2>
                  ...
                  {['Daily', 'Weekly', 'Monthly', 'Never'].map((option) => (
                    <div
                      ...
                      className={`px-4 py-2  cursor-pointer hover:bg-gray-700 ${
                        settings.notification.schedule === option ? 'text-[#F9BC07]' : 'text-gray-300'
                      }`}
        ...
        <section className="mb-6 px-4 md:px-0" aria-labelledby="reminder-title">
          <h2 id="reminder-title" className={`text-[24px] md:text-[28px] mb-1 ${
            settings.theme.lightMode ? 'text-black' : 'text-[#CFFDED]'
          }`}>
            Reminder Me
          </h2>
              ...
              <div className="absolute z-10 w-full mt-1 bg-[#1E1E1E] border border-gray-700 rounded-md shadow-lg">
                <div className="py-1">
                  {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map((day) => (
                    <div
                      key={day}
                      className={`px-4 py-2 text-sm cursor-pointer text-[#717171] text-[15px] hover:bg-gray-700 ${
                        settings.reminder.day === day ? 'text-[#F9BC07]' : 'text-[#717171]'
                      }`}
                      onClick={() => {
                        setSettings({
                          ...settings,
        ...
        <section className="mb-6 px-4 md:px-0" aria-labelledby="theme-title">
          <h2 id="theme-title" className={`text-[24px] md:text-[28px] mb-1 ${
            settings.theme.lightMode ? 'text-black' : 'text-[#CFFDED]'
          }`}>
            Theme
          </h2>
        ...
        <section className="mb-6 px-4 md:px-0" aria-labelledby="sound-title">
          <h2 id="sound-title" className={`text-[24px] md:text-[28px] mb-4 ${
            settings.theme.lightMode ? 'text-black' : 'text-[#CFFDED]'
          }`}>
            Sound
          </h2>
